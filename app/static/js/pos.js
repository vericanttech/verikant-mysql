...
function updateCartDisplay() {
    ...
    cartItems.innerHTML = ''; // Re-use the existing cartItems variable
    let total = 0;
    ...
    cartTotal.textContent = `${formatNumberFR(total.toFixed(2))} ${currency}`; // Re-use the existing cartTotal variable
    checkoutBtn.disabled = window.cart.length === 0; // Re-use the existing checkoutBtn variable
...
if (savedCart) {
    ...
    document.getElementById('cash-received').addEventListener('input', function () {
        ...
        const received = parseFloat(cashReceived.getRawValue()) || 0;
    ...
    document.getElementById('complete-sale-btn').addEventListener('click', async function() {
        try {
            ...
            const cashReceivedValue = cashReceived.getRawValue();
